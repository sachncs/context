...
Playbooks grow by ADD (the only operation ACE implements per the
upstream repo), and shrink by deterministic dedup when context is
over budget.
...
- :func:`parse_playbook` / :func:`render_playbook` — round-trip with
  the line-based on-disk format
- :class:`Evolver` — the Generator → Reflector → Curator loop
  (in ``ceng/playbook/evolver.py``)

The verbatim Generator / Reflector / Curator prompts from the ACE
GitHub repo (Stanford, SambaNova, UC Berkeley) live in
``ceng/playbook/prompts.py`` so they're easy to audit and to update
...
class Playbook:
    ...
    A ``Playbook`` is reconstructable to text via ``render_for_context``
    or to a slice of bullets via ``get_active``.
    """
    ...
    def add_bullet(self, bullet: Bullet) -> None:
        """Append a bullet. Existing id is overwritten with the new one."""
        if bullet.section not in self.sections_in_order:
            self.sections_in_order.append(_canon_section(bullet.section))
    ...
    def merge(self, incoming: Iterable[Bullet], *, dedup_threshold: float = 0.90) -> int:
        ...
        Two-pass:
        1. Bullet-id collision: keep the higher net-score bullet.
        2. Content-hash dedup: if two bullets have the same
           normalised-content SHA-256, keep the higher net-score.
           Defaults ``dedup_threshold`` to 0.90 (paper §A.6 best).
        ...
        for bullet in incoming:
            ...
            if existing is not None:
                if bullet.net_score > existing.net_score:
                    self.bullets[bullet.id] = bullet
                continue
            norm = _content_canonical(bullet.content)
            dup_id = self._find_duplicate_by_content(norm)
    ...
    def _find_duplicate_by_content(self, canonical: str) -> str | None:
        for bid, b in self.bullets.items():
            if _content_canonical(b.content) == canonical:
                return bid
        return None
    ...
    def trim_to_token_budget(
        self, tokenizer: "object | None" = None, budget: int = 80_000
    ) -> int:
...
def _content_canonical(content: str) -> str:
    import hashlib
